        ...
        const timer = setInterval(() => {
            ...
            for (const [ip, times] of this.windows) {
                ...
                if (trimmed.length === 0)
                    ...
                    this.windows.set(ip, trimmed);
            }
        }, Math.max(this.windowMs, 60000));
...
let webhookNotifier: WebhookNotifier | null = runtimeConfig.webhookUrl
    ...
    : null;
const telegramNotifier: TelegramNotifier | null = APP_CONFIG.telegramBotToken && APP_CONFIG.telegramChatId
...
function rejectUnauthorized(req: IncomingMessage, res: ServerResponse, target: 'mcp' | 'api', path: string, requiredRole: Exclude<ApiRole, 'none'>, asJsonRpc: boolean): boolean {
...
function ensureAuthorizedApi(req: IncomingMessage, res: ServerResponse, pathname: string, requiredRole: Exclude<ApiRole, 'none'>): boolean {
...
function parseWaitApiPath(pathname: string): {
    id: string;
    action: 'respond' | 'cancel' | 'extend';
} | null {
...
function parseHistoryResolution(value: string | null): 'message' | 'timeout' | 'canceled' | undefined {
    if (!value) {
        return undefined;
    }
...
async function handleApiRequest(req: IncomingMessage, res: ServerResponse, pathname: string): Promise<void> {
    ...
    if (req.method === 'GET' && pathname === '/api/auto-configure-status') {
        ...
        for (const t of targets)
            status[t] = checkClientStatus(t, serverUrl);
    ...
    if (req.method === 'GET' && /^\/api\/waits\/[^/]+$/.test(pathname)) {
        ...
        if (!wait) {
            sendJson(res, 404, { error: 'wait-not-found' });
            return;
        }
...
httpServer.listen(APP_CONFIG.httpPort, APP_CONFIG.httpHost, () => {
    ...
    logger.info(`API key roles: operator=${APP_CONFIG.operatorApiKey ? 'set' : 'unset'}, viewer=${APP_CONFIG.viewerApiKey ? 'set' : 'unset'}`);
